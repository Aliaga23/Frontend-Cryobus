...
const GestionTipoEnvio = () => {
  ...
  const fetchTipoEnvios = async () => {
    try {
      const response = await axios.get('https://proyecto2-production-ba5b.up.railway.app/api/tipoEnvio');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editTipoEnvio) {
        await axios.put(`https://proyecto2-production-ba5b.up.railway.app/api/tipoEnvio/${editTipoEnvio.ID}`, newTipoEnvio);
        setEditTipoEnvio(null);
      } else {
        await axios.post('https://proyecto2-production-ba5b.up.railway.app/api/tipoEnvio', newTipoEnvio);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://proyecto2-production-ba5b.up.railway.app/api/tipoEnvio/${id}`);
      fetchTipoEnvios();
    } catch (error) {
