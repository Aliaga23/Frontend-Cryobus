...
const GestionPermisoRol = () => {
  ...
  const fetchPermisos = async () => {
    try {
      const response = await axios.get('https://proyecto2-production-ba5b.up.railway.app/api/permisos');
  ...
  const fetchRolPermisos = async (idRol) => {
    try {
      const response = await axios.get(`https://proyecto2-production-ba5b.up.railway.app/api/permisoRol/${idRol}`);
  ...
  const handleAssignPermiso = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`https://proyecto2-production-ba5b.up.railway.app/api/permisoRol/${selectedRol}`, newPermiso);
  ...
  const handleRemovePermiso = async (nro) => {
    try {
      await axios.delete(`https://proyecto2-production-ba5b.up.railway.app/api/permisoRol/${selectedRol}/${nro}`);
      fetchRolPermisos(selectedRol);
    } catch (error) {
